...
export default function CompatByIdPage() {
  const router = useRouter()
  const params = useParams<{ id: string }>()
  const compatId = decodeURIComponent(String(params?.id ?? ""))
  ...
  useEffect(() => {
    ...
    compatApi.get(compatId)
      .then(data => { if (!cancelled) setResult(data) })
      .catch((e: any) => {
        if (cancelled) return
        const status = e?.response?.status
        if (status === 404) {
          setError("この相性診断結果は見つかりませんでした。")
        } else {
  ...
  if (error) {
    return (
      <div className="relative min-h-screen flex items-center justify-center pb-28">
        <Stars />
        <div className="relative z-10 text-center px-6">
          <div className="font-serif text-[16px] text-white/80 mb-3">{error}</div>
      ...
      <div className="relative z-10 max-w-app mx-auto px-[18px]">
        ...
        {outputs?.scores && (
          <div className="relative overflow-hidden rounded-2xl p-6 pt-10 mt-3 backdrop-blur-sm" style={GOLD_CARD_STYLE}>
            <img src="/asteria/assets/compat-corner-constellation.png" alt="" aria-hidden
            ...
            <div className="relative z-[1] text-center">
              ...
              <h2 className="mt-3 font-serif text-[24px] leading-tight text-[#F6F1E4]"
            ...
            {scoreComment && (
              <div className="mt-5 flex items-start gap-3 rounded-2xl border border-[#C9A554]/35 px-4 py-3.5"
                style={{ background: "rgba(8,12,30,0.55)" }}>
                <span className="mt-1 shrink-0 text-[14px] text-[#F0C870]"
        ...
        {outputs?.advice && (
          <>
            ...
            <div className="rounded-2xl p-4"
              ...
              <p className="font-serif text-[14px] leading-[1.85] text-[#D8E8D0] font-light">
                {adviceText}
              </p>
            </div>
          </>
        ...
        {outputs?.highlight && (
          <div className="mt-2.5 rounded-2xl p-4 bg-[#C9A554]/10 border border-[#C9A554]/30">
            ...
            <p className="font-serif text-[14px] leading-[1.85] text-[#E8E0C8] font-light">
              {outputs.highlight}
            </p>
          </div>
        )}
        ...
        {outputs?.conflict_scene && (
          <>
            ...
            <div className="rounded-2xl p-4"
              ...
              <p className="font-serif text-[14px] leading-[1.85] text-[#E8D8C8] font-light">
                {outputs.conflict_scene}
              </p>
            </div>
          </>
